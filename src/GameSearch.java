...
public abstract class GameSearch {
    ...
    public abstract float           positionEvaluation( Position p, boolean player);/*Returns a numerical evaluation of a given board position*/
    ...
    protected ArrayList minMax(int depth, Position p, boolean player) {
        ...
        if(GameSearch.DEBUG) System.out.println("^^ al(0): " + al.get(0) + ", al(1): " + al.get(1));
        return al;
    }
    ...
    protected ArrayList minMaxHelp(int depth, Position p, boolean player, float alpha, float beta) {
        ...
        if (maxDepth(p, depth)) {
            ...
            if(cutoff > optimalMoves) value = -value;
