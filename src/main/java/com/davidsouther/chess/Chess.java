...
public class Chess extends GameSearch {
    ...
    /*
     * Decides if Position p is a stalemate. If all possible moves from p are checks
    ...
    /*
     * Determins if Position p is won for Player player. simply a helper method from
     * game search.
     * 
     * Warning: checkMate determines whether Player player is mated, this determines
     * if player has won.
     *
    ...
    public boolean wonPosition(Position p, boolean player) {
        if (checkMate((ChessPosition) p, !player))
            return true;
        return false;
    }

    /*
     * Determines if player's king is attacked at this position. Used extensivly by
     * checkMate and findPossibleMoves
     *
    ...
    /*
     * Determines if player has no possible legal moves at ChessPosition p.
     *
     * @param ChessPosition p ChessPosition to be considered.
     * 
    ...
    /*
     * Returns float value of Position p for player Combines material data, control
     * data, attack data, and center control.
     *
    ...
    public static Position makeMove(Position p, boolean player, Move move) {
        if (GameSearch.DEBUG)
            System.out.println("Entered Chess.makeMove");
    ...
    public void printControlData(ChessPosition p) {
        ...
        for (int i = 99; i >= 22; i--) {
            if (b[i] == 7 && b[i + 1] == 7)
                System.out.println();
            if (b[i] != 7)
                System.out.print(i + ": (" + b[i] + ") " + computerControl[i] + " " + humanControl[i] + "\t");
        }
        System.out.println("");
            ...
            switch (piece_type) {
                case ChessPosition.PAWN:
                    ...
                    for (int delta = -1; delta <= 1; delta += 2) {
                        ...
                        if ((target <= -1 && target != 7 && piece > 0) || (target >= 1 && target != 7 && piece < 0)) {
                ...
                case ChessPosition.QUEEN:
                    ...
                    outer: while (true) {
                        inner: while (true) {
                            ...
                            // if(((piece > 0) && (b[next_square] < 0)) || ((piece < 0) && (b[next_square] >
                            // 0)))break inner;
            ...
            if ((board_val < 0 && !player) || (board_val > 0 && player)) {
                ...
                for (int j = 0; j < num; j++) {
                    if (b[piece_moves[j]] != 7) {
                        ...
                        if (DEBUG)
                            System.out.println(
                                    "possible move: player=" + player + ", from=" + i + ", to=" + piece_moves[j]);
                        count++;
                    }
                }
        ...
        switch (piece_type) {
            case ChessPosition.PAWN: {
                ...
                if (piece > 0)
                    ...
                    temp = 8;
                if (b[move_offset] == 0 && (square_index / 10) == temp
                        && ((piece < 0 && b[square_index - 10] == 0) || (piece > 0 && b[square_index + 10] == 0))) {
    ...
    public static int[] initialBoard = { 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7,
            ChessPosition.ROOK, ChessPosition.KNIGHT, ChessPosition.BISHOP, ChessPosition.QUEEN, ChessPosition.KING,
            ChessPosition.BISHOP, ChessPosition.KNIGHT, ChessPosition.ROOK, 7, 7, // white pieces
            ChessPosition.PAWN, ChessPosition.PAWN, ChessPosition.PAWN, ChessPosition.PAWN, ChessPosition.PAWN,
            ChessPosition.PAWN, ChessPosition.PAWN, ChessPosition.PAWN, 7, 7, // white pawns
            ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK,
            ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, 7, 7, // 8 blank squares, 2 off board
            ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK,
            ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, 7, 7, // 8 blank squares, 2 off board
            ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK,
            ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, 7, 7, // 8 blank squares, 2 off board
            ...
            ChessPosition.BLANK, ChessPosition.BLANK, ChessPosition.BLANK, 7, 7, // 8 blank squares, 2 off board
            -ChessPosition.PAWN, -ChessPosition.PAWN, -ChessPosition.PAWN, -ChessPosition.PAWN, -ChessPosition.PAWN,
            -ChessPosition.PAWN, -ChessPosition.PAWN, -ChessPosition.PAWN, 7, 7, // black pawns
            -ChessPosition.ROOK, -ChessPosition.KNIGHT, -ChessPosition.BISHOP, -ChessPosition.QUEEN,
            -ChessPosition.KING, -ChessPosition.BISHOP, -ChessPosition.KNIGHT, -ChessPosition.ROOK, 7, 7, // black
                                                                                                          // pieces
            7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7 };

    private static int[] index = { 0, 12, 15, 10, 1, 6, 0, 0, 0, 6 };

    private static int[] pieceMovementTable = { 0, -1, 1, 10, -10, 0, -1, 1, 10, -10, -9, -11, 9, 11, 0, 8, -8, 12, -12,
            19, -19, 21, -21, 0, 10, 20, 0, 0, 0, 0, 0, 0, 0, 0 };
    private static int[] value = { 0, 1, 3, 3, 5, 9, 0, 0, 0, 200 };
    public static int[] blackSquares = { 22, 24, 26, 28, 33, 35, 37, 39, 42, 44, 46, 48, 53, 55, 57, 59, 62, 64, 66, 68,
            73, 75, 77, 79, 82, 84, 86, 88, 93, 95, 97, 99 };
}
